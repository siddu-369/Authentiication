...
import { mailTrapClient } from "../MailTrap/mailTrap.config.js";
...
export const signup = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      ...
      user: {
        ...user._doc,
        password: undefined, //to prevent password from being sent in response
      },
    });
  } catch (err) {
...
export const verifyEmail = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid or code expired" });
    }
...
export const forgotPassword = async (req, res) => {
  ...
  try {
    ...
    const resetsTokenExpiresAt = Date.now() + 1 * 60 * 60 * 1000; // 1 hour

    user.resetPasswordToken = resetToken;
    user.resetPasswordExpiresAt = resetsTokenExpiresAt;
    await user.save();
